"""
Text-JEPA Linear Probing Evaluation (CLS-based)

Evaluates a pretrained Text-JEPA encoder by LINEAR PROBING:
the encoder is FROZEN and only the MLP head is trained on AG News.
"""
...
class TextLinearProbeModel(nn.Module):
    ...
    Encoder is FROZEN - only the MLP head is trainable.
    """
...
class CSVLogger:
    def __init__(self, output_dir="outputs/text_jepa"):
        ...
        self.csv_path = os.path.join(
            output_dir, f"linearprobe_results_{self.timestamp}.csv"
        )
...
def linearprobe_encoder(
    ...
    assert tokenizer.cls_token_id is not None, "Tokenizer must have a CLS token"
    ...
    # Optimizer: only trains the head (norm + classifier)
    ...
    print(f"Linear probing with head lr={lr} (encoder is frozen)")
    ...
    # Save final model (head only is sufficient, but saving full model for convenience)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    model_save_path = os.path.join(output_dir, f"linearprobe_model_{timestamp}.pth")
    ...
    with open(logger.txt_path, "w") as f:
        f.write("=" * 70 + "\n")
        f.write("TEXT-JEPA LINEAR PROBING TRAINING LOG\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write(f"  Linear({embed_dim} -> {num_classes})  ← trainable\n")
        f.write(")\n\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Text-JEPA Linear Probing (CLS)")
